...
import { setMovieList } from '../../redux/actions/QuanLyCumRapAction';
...
export default function Detail(props) {
  ...
  const { filmDetail } = useSelector(state => state.QuanLyCumRapReducer)
  const dispatch = useDispatch()
  useEffect(()=>{
    let {id} = props.match.params
    dispatch(setMovieList(id))
  },[])
  ...
  return (
    <div style={{
      ...
      >
        ...
        {isMobile ? (<div className="container mt-5 boxSadow">
          <h1 className="text-center">Lịch chiếu</h1>
          <Tabs centered defaultActiveKey="1">
            <TabPane tab="Lịch chiếu" key="1">
              <Tabs centered tabPosition={tabPositionTop}>
                {filmDetail.heThongRapChieu?.map((htr, index) => {
                  return <TabPane tab={<div style={{ textTransform: "uppercase" }}><img src={htr.logo} className="me-3" width="35" height="35" />{htr.tenHeThongRap}</div>} key={index}>
                        ...
                        <Grid container className="mb-5 pl-2" >
                          <Grid md={12} sm={12}>
                            <div style={{ display: "flex", marginBottom: 10 }}>
                              ...
                              <div style={{ color: "gray", marginBottom: 15 }} className="ms-3" >
            ...
            <h1 className="text-center mt-5">Lịch chiếu</h1>
          <Tabs centered defaultActiveKey="1">
                  ...
                  return <TabPane tab={<div style={{ textTransform: "uppercase" }}><img src={htr.logo} className="me-3" width="35" height="35" />{htr.tenHeThongRap}</div>} key={index}>
                    {htr.cumRapChieu?.map((cumRap, index) => {
                      return <div key={index}>
                        ...
                        <Grid container className="mb-5 pe-2" >
                          <Grid md={12} sm={12}>
                            <div style={{ display: "flex", marginBottom: 10 }}>
                              <img src={cumRap.hinhAnh} width="50" height="50px" />
                              <div style={{ color: "gray", marginBottom: 15 }} className="ms-3" >
