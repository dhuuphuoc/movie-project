...
export default function News() {
    ...
    const { arrNews, arrSell } = useSelector(state => state.QuanLyCumRapReducer)
        ...
        return arrNews.map((movie, index) => {
            return <Grid item xs={12} md={6} key={index}>
                <a className="dienanh">
                    <div>
                        <h6>{movie.tenPhim}</h6>
                        <p style={{color:"black"}}>{movie.moTa.length > 100 ? <span>{movie.moTa.slice(0, 100)}...</span> : <span>{movie.moTa}</span>}</p>
                    </div>
    ...
    const renderReview = () => {
        return arrNews.slice(0, 6).map((review, index) => {
            return <Grid item xs={12} md={6} key={index}>
